...
const PrebidLogs = () => {
	...
	useEffect(() => {
		...
		const handleLog = (event) => {
			...
			setLogs((prev) => [
				...prev,
				{ eventName: event.type, data: event.detail },
			]);
		...
		if (Array.isArray(events)) {
			events.forEach((eventName) => {
				if (typeof eventName === "string") {
					window.addEventListener(eventName, handleLog);
				}
			});
		}
		...
		return () => {
			if (Array.isArray(events)) {
				events.forEach((eventName) => {
					if (typeof eventName === "string") {
						window.removeEventListener(eventName, handleLog);
					}
				});
			}
		};
	...
	return (
		<div className="flex flex-col lg:flex-row gap-4 p-4 min-h-screen">
			...
			<div className="flex-1 bg-gray-100 p-4 rounded-lg overflow-y-auto h-[80vh]">
				...
				{logs.length === 0 && (
					<p className="text-gray-500">Поки що немає подій...</p>
				)}
				{logs.map((log, idx) => (
					<div
						key={`${log.eventName}-${idx}`}
